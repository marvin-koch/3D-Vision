...
def sobel_line(sobel_depth, sobel_normal, valid_mask, line, trim_ratio=0.25):
    """
    Computes the Sobel response in a trimmed neighborhood along a given line, considering only valid pixels.

    Parameters:
    sobel_depth: 2D array of Sobel-filtered depth image.
    sobel_normal: 2D array of Sobel-filtered normal image.
    valid_mask: 2D binary mask indicating valid pixels (1 for valid, 0 for invalid).
    ...
    # Create masks for the line
    mask_depth = np.zeros_like(sobel_depth, dtype=np.uint8)
    mask_normal = np.zeros_like(sobel_normal, dtype=np.uint8)
    cv2.line(mask_depth, (x1, y1), (x2, y2), 1, 1)
    cv2.line(mask_normal, (x1, y1), (x2, y2), 1, 1)

    # Apply valid_mask
    mask_depth = mask_depth * valid_mask
    mask_normal = mask_normal * valid_mask
