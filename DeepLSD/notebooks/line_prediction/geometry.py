...
def get_line_pixels(line, valid_mask, maps):
    """
    ...
    y_coords, x_coords = np.where((blank_image == 255) & (valid_mask > 0))
    return list(zip(x_coords, y_coords))

def get_line_pixels_trim(line, valid_mask, maps, trim_ratio=0.25):
    """
    Get all pixel coordinates along a line using cv2.line.
    """
