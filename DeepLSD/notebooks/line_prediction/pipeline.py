...
import numpy as np
from line_prediction.geometry import calculate_plane_for_map, get_line_pixels, get_line_pixels_trim
from line_prediction.clustering import cluster_coplanar_points, find_line_planes
...
def prediction_pipeline(image_id, frame_str, net, device, 
    ...
    segmentation_map, original_map = cluster_coplanar_points(
        plane_map, world_coordinates, valid_mask, cluster_selection_epsilon=cluster_selection_epsilon, min_cluster_size=min_cluster_size#0.01, sample_rate=1, threshold=1 #0.02
    )

    # Determine coplanarity label for each line.
    all_coplanarity_labels = find_line_planes(pred_lines, segmentation_map,valid_mask, get_line_pixels_trim)
